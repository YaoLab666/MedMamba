    ...
    with torch.no_grad():
        ...
        if model.is_generative and eval_gen_config is not None:
            ...
            if is_conditional and is_supervised:
                ...
                # The batch target is unavailable here; infer condition from cached results when possible.
                if loader_config is None:
                    logger.warning('No loader provided, cannot infer conditional input for generation. Set cond=None.')
                    cond = None
                else:
                    try:
                        _cond_arr = extract_results(results, 'condition')
                        if _cond_arr is None:
                            _cond_arr = extract_results(results, 'target')
                        cond = torch.from_numpy(_cond_arr[0]) if _cond_arr is not None else None
                    except Exception:
                        cond = None
            elif is_supervised:
                # The batch input is unavailable here; infer condition from cached inputs when possible.
                if loader_config is None:
                    logger.warning('No loader provided, cannot infer supervised input for generation. Set cond=None.')
                    cond = None
                else:
                    try:
                        _inp_arr = extract_results(results, 'input')
                        cond = torch.from_numpy(_inp_arr[0]) if _inp_arr is not None else None
                    except Exception:
                        cond = None
            sampler = None
