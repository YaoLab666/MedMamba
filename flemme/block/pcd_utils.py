...
def _require_pcd_ops():
    if pcd_ops is None:
        raise ImportError(
            "cpp_extension.pcd_ops is required for this point-cloud operator. "
            "Rebuild the CUDA extension for the active Python/CUDA environment."
        )
    return pcd_ops
...
class FurthestPointSampling(Function):
    ...
    def forward(ctx, xyz, npoint):
        ...
        """
        out = _require_pcd_ops().furthest_point_sampling(xyz, npoint)
...
# Global flag used to print the gather warning once.
_GATHER_WARNING_PRINTED = False

def gather_features(features, index, channel_dim, gather_dim):
    global _GATHER_WARNING_PRINTED
    
    # Normalize negative dimensions.
    if gather_dim < 0:
        gather_dim = features.ndim + gather_dim
    
    # Clamp out-of-range indices to avoid gather failures.
    max_index = features.shape[gather_dim] - 1
    
    # Print the warning only once.
    if not _GATHER_WARNING_PRINTED and (torch.any(index < 0) or torch.any(index > max_index)):
        print(f"\n{'='*60}")
        print("[WARNING] gather_features: index out of bounds detected.")
        print(f"{'='*60}")
        print(f"  features.shape: {features.shape}, gather_dim: {gather_dim}")
        print(f"  index.shape: {index.shape}")
        print(f"  index range: [{index.min().item()}, {index.max().item()}]")
        print(f"  valid range: [0, {max_index}]")
        invalid_count = ((index < 0) | (index > max_index)).sum().item()
        print(f"  invalid indices count: {invalid_count}")
        print("\n  Indices are being automatically clamped to the valid range.")
        print("  This warning will only show once.")
        print(f"{'='*60}\n")
        _GATHER_WARNING_PRINTED = True
...
class ThreeNN(Function):
    ...
    def forward(ctx, unknown, known):
        ...
        """
        dist2, idx = _require_pcd_ops().three_nn(unknown, known)
...
class ThreeInterpolate(Function):
    ...
    def forward(ctx, features, idx, weight):
        ...
        return _require_pcd_ops().three_interpolate(features, idx, weight)
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = _require_pcd_ops().three_interpolate_grad(
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features, idx):
        ...
        return _require_pcd_ops().group_points(features, idx)
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = _require_pcd_ops().group_points_grad(grad_out.contiguous(), idx, N)
...
class BallQuery(Function):
    ...
    def forward(ctx, radius, nsample, xyz, new_xyz):
        ...
        """
        output, dist = _require_pcd_ops().ball_query(new_xyz, xyz, radius, nsample)
...
class BatchBallQuery(Function):
    ...
    def forward(ctx, radius, nsample, xyz, new_xyz):
        ...
        torch.Tensor
            (B, npoint, nsample) tensor with the indicies of the features that form the query balls
        """
        output, dist = _require_pcd_ops().batch_ball_query(new_xyz, xyz, radius, nsample)
        ctx.mark_non_differentiable(output)
        ctx.mark_non_differentiable(dist)
        return output, dist
...
class TrilinearDevoxelization(Function):
    ...
    def forward(ctx, features, coords, resolution, is_training=True):
        ...
        outs, inds, wgts = _require_pcd_ops().trilinear_devoxelize_forward(resolution, is_training, coords, features)
        if is_training:
    ...
    def backward(ctx, grad_output):
        ...
        grad_inputs = _require_pcd_ops().trilinear_devoxelize_backward(grad_output.contiguous(), inds, wgts, ctx.r)
...
class AvgVoxelization(Function):
    ...
    def forward(ctx, features, coords, resolution):
        ...
        out, indices, counts = _require_pcd_ops().avg_voxelize_forward(features, coords, resolution)
    ...
    def backward(ctx, grad_output):
        ...
        grad_features = _require_pcd_ops().avg_voxelize_backward(grad_output.contiguous().view(b, c, -1), indices, counts)
