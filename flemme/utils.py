...
if module_config['point-cloud'] or module_config['graph']:
    ...
    def _require_plyfile():
        if PlyData is None or PlyElement is None:
            raise ImportError(
                "plyfile is required to read or write PLY point clouds. "
    ...
    def _require_trimesh():
        if trimesh is None:
            raise ImportError(
                "trimesh is required to read mesh point-cloud formats. "
    ...
    def load_ply(inputfile, vertex_features = None, with_edges = False, with_faces = False):
        PlyDataLocal, _ = _require_plyfile()
        plydata = PlyDataLocal.read(inputfile, known_list_len={'face': {'vertex_indices': 3}})
    ...
    def save_ply(filename, points, edges = None, faces = None):
        PlyDataLocal, PlyElementLocal = _require_plyfile()
        pcolors = None
    ...
    def load_pcd(filename):
        ...
        if suffix == 'ply':
            ...
            mesh = _require_trimesh().load_mesh(filename)
